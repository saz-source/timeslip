...
class QueueManager(tk.Toplevel):
    ...
    def _build(self):
        ...
        self._error_lbl = tk.Label(self, textvariable=self._error_var,
                                    font=FONT_SM, bg=BG, fg="#cc0000",
                                    anchor=tk.W, wraplength=720, justify="left")
        self._error_lbl.pack(fill=tk.X, padx=18, pady=(0, 4))
        ...
        self._retry_btn = mac_btn(bf, "  Retry Now  ", self._retry_selected,
                                   primary=True)
    ...
    def _on_retry_success(self, company_name: str):
        self._refresh_table()
        self._refresh_banner()
        messagebox.showinfo("Submitted", f"Entry for {company_name} submitted successfully.")

    def _on_retry_fail(self, item_id: str, error: str):
